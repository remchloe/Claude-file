...
> **定位说明**：本工具产出的是**按时间排列的流水报告**（数据整理 + 互动数据 + 原文链接）。
> 它**不做话题聚类和观点综合**——那属于分析层，需要另行加工
> （参见 `reports/weibo/王虎的舰桥_微博报告/` 的"观察报告"，是在本工具数据基础上二次综合的产物）。
...
python scripts/weibo/weibo_report.py "王虎的舰桥" 2026-07-21 --comments   # 附带热门评论
```
...
| 选项 | 说明 |
|------|------|
| `--comments` | 额外抓取重点帖（长文 + 高互动）的热门评论与**博主本人回复**，默认关闭 |

> 评论默认关闭：实测评论区以玩梗/反讽为主，信噪比低。
> 值得注意的是**博主本人在评论区的补充**往往是有效信息（论据补充、勘误），
> 开启后这部分会被单独标出（`🔸 博主补充`）。
...
- **评论区**：`comments/hotflow`，每帖取 2 页热门评论（约 40 条）。
  仅对**长文帖 + 互动 TOP30** 抓取（全量抓取信噪比过低）。
- **限速**：翻页间隔 2.5s、长文 1.2s、评论 0.8s，避免触发风控。
